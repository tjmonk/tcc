    ...
    ; ensure /SYS/TEST/C max length is 8
        JMP G_O
    notice
        DAT "External Variable Validation test\n"
    sig
        DAT "Received signal: "
    intvar
        DAT "/SYS/TEST/A"
    ...
    G_O
        MOV R6,0
        MOV R7,10      ; max value for /sys/test/a
        MOV R0,notice
    ...
    WAIT
        ...
        MOV R13,sig
        WRS R13        ; output received signal notification
        WRN R3         ; output signal number
        WRC '\n'
        MOV R5,42      ; R5 (42) is code for the validation signal
        CMP R3,R5      ; see if we received the validation signal
        JNZ WAIT       ; we are not handling this signal
        MOV R7,34      ; set validation response value to ERANGE
        ...
        MOV R7, 2      ; set validation response value to ENOTSUP
        JMP ENDVAL     ; not our variable
    validate_a
    ...
    validate_c
        MOV R9,1
        CSB R9         ; Create a new string buffer (1)
        GET.S R9,R2    ; get the variable into the string buffer
